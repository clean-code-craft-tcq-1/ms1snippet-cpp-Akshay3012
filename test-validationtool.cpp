...
TEST_CASE("reports error when there is no reading of soc") { 
  ...
  REQUIRE(validateSOCreadings(socReadings_empty, numOfSocReadings_empty) == false);
}
...
TEST_CASE("reports error when there is no reading of current") { 
  ...
  REQUIRE(validateSOCreadings(currentReadings_empty, numOfCurReadings_empty) == false);
}
